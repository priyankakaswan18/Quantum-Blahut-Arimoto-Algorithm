...
if strcmp(quantity,'Holevo_quantity')
    output=ketbra(zero,zero)*trace( ERand(ketbra(zero,zero))*( logm(ERand( eye(2)*10^(-6) + ketbra(zero,zero)))./ log(2)-logm(ERand(sigma))./ log(2) ) )...
        +ketbra(one,one)*trace( ERand(ketbra(one,one))*( logm(ERand( eye(2)*10^(-6) +ketbra(one,one)))./ log(2)-logm(ERand(sigma))./ log(2) ) );
end
